...
class QPCVXPY:
    def __init__(self, prob: Problem) -> None:
        ...
        self.neq = (self.horizon - 1) * self.nx + self.nx  # integration + Initial state
        if self.prob.stage_dynamics[0].type == "inverse":
            ng = self.prob.stage_dynamics[0].ng
            self.neq += (self.horizon - 1) * ng  # underactuation

        z = cp.Variable(self.nvars)
        Q = cp.Parameter((self.nvars, self.nvars))
        p = cp.Parameter(self.nvars)
        A = cp.Parameter((self.neq, self.nvars))
        b = cp.Parameter(self.neq)

        # objective = cp.Minimize(z.T @ Q @ z + p.T @ z)
        objective = cp.Minimize(cp.norm(Q @ z - p))
        constraints = []
        constraints += [A @ z == b]
